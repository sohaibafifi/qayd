...
the locations (``list_vars(1, facilities)`` puts every facility in the one list).
...
model = cp.Model()
(p,) = model.list_vars(1, list(range(n)))  # p[i] = facility placed at location i
