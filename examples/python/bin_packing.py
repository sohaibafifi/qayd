...
W = cp.array(weight)
model = cp.Model()
bins = model.list_vars(n, items)                       # up to n bins available
...
used_bins = [b for b in solution.routes if b]
...
assert sorted(i for b in solution.routes for i in b) == items, "every item packed once"
for b in solution.routes:
