"""RCPSP/MRCPSP scheduling with an optional PSPLIB .sm/.mm instance.

Without a positional file this keeps the deterministic generated RCPSP. Pass a
PSPLIB file to solve it directly:

    uv run examples/python/scheduling/native/rcpsp.py j301_1.sm --threads 4
    uv run examples/python/scheduling/native/rcpsp.py c151_1.mm --engine auto
"""
...
parser.add_argument("instance", nargs="?", default=os.environ.get("QAYD_RCPSP_INSTANCE"))
parser.add_argument("--tasks", type=int, default=int(os.environ.get("QAYD_RCPSP_N", "12")))
parser.add_argument("--resources", type=int, default=int(os.environ.get("QAYD_RCPSP_R", "2")))
parser.add_argument("--time-limit", type=int, default=int(os.environ.get("QAYD_RCPSP_T", "8")))
parser.add_argument("--threads", type=int, default=int(os.environ.get("QAYD_RCPSP_THREADS", "1")))
parser.add_argument("--seed", type=int, default=int(os.environ.get("QAYD_RCPSP_SEED", "0")))
...
if multi_mode:
    ...
    intervals = model.alternatives([[(0, mode["duration"])] for mode in chosen_modes], horizon)
    index_of = {job: index for index, job in enumerate(job_ids)}
    for job in job_ids:
        for successor in successors[job]:
            model.precedence(intervals[index_of[job]], intervals[index_of[successor]])
    for resource, (kind, capacity) in enumerate(zip(resource_kinds, capacities)):
        if kind in {"renewable", "doubly_constrained"}:
            demands = [
                (intervals[index], mode["demands"][resource])
                for index, mode in enumerate(chosen_modes)
                if mode["demands"][resource] > 0
            ]
            if demands:
                model.resource(demands, capacity)
