...
picks the collection engine because the model uses a list variable.
...
model = cp.Model()
(tour,) = model.list_vars(1, list(range(1, n + 1)))    # one list = one tour over the cities
...
print(f"tour: {[0, *solution.routes[0], 0]}")
assert sorted(solution.routes[0]) == list(range(1, n + 1)), "every city visited once"
