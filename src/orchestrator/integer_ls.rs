...
use crate::engines::ls::cop::{solve_ls_capped, solve_ls_capped_borrowed, LocalRhs, LocalSearchOutcome, LocalSearchSpec, LsConfig};
...
pub(crate) struct IntegerLocalSearchPlan {
    ...
    warm_start: Option<IntegerWarmStartKind>,
...
pub(crate) fn warm_start(
    ...
    match plan {
        IntegerWarmStartPlan::Local(plan) => local_warm_start(model, compiled, plan, request, budget, engine_stop),
        IntegerWarmStartPlan::ScenarioSchedule(plan) => scenario_schedule_warm_start(model, compiled, plan, request, budget, engine_stop),
        IntegerWarmStartPlan::DisjunctiveSchedule(plan) => {
            disjunctive_schedule_warm_start(model, compiled, plan, request, budget, engine_stop)
        }
        IntegerWarmStartPlan::Fallbacks(plans) => {
            for plan in plans {
                if budget.expired() || engine_stop.load(std::sync::atomic::Ordering::Acquire) {
                    return Ok(None);
                }
                if let Some(candidate) = warm_start(model, compiled, plan, request, budget, engine_stop)? {
...
fn scenario_schedule_warm_start(
    ...
    let started = Instant::now();
    let Some(solution) = plan.construct(model, engine_stop, ConstructionLimits::default()) else {
        return Ok(None);
    };
    let Some(repaired) =
        repair_semantic_candidate(model, compiled, &solution.values, request, budget, request.seed, VerificationLevel::Transfer)?
    else {
        return Ok(None);
    };
    let physical_objective = repaired
...
fn disjunctive_schedule_warm_start(
    ...
    let Some(construction) = plan.construct(model, request.seed, engine_stop, construction_budget) else {
        return Ok(None);
    };
...
fn repair_semantic_candidate(
    ...
    repair_partial_semantic_candidate(model, compiled, &values, request, budget, seed, verification)
}
...
fn repair_partial_semantic_candidate(
    ...
    if budget.expired() && verification == VerificationLevel::Transfer {
        return Ok(None);
    }
    ...
    let completed = if problem.search.iter().all(|variable| solver.store.is_fixed(*variable)) {
        ...
        let conflict_limit = Some(request.limits.conflicts.unwrap_or(10_000).min(10_000));
        let (solution, _, complete) = crate::search::decide_sat_assuming_seeded(
            &mut solver,
            &problem.search,
            &[],
    ...
    let physical_objective = physical_objective_value(problem, &completed)?;
    let candidate = super::cp::candidate_if_running(model, compiled, &completed, budget, verification)?.map(|candidate| {
        CandidateSolution::verified(
            candidate.assignment().clone(),
            candidate.objectives().to_vec(),
            EngineKind::IntegerLocalSearch,
    ...
    if let Some(candidate) = &candidate {
        super::cp::verify_assumptions(candidate, &request.assumptions)?;
    }
    Ok(candidate.map(|candidate| RepairedCandidate { candidate, physical_solution: completed, physical_objective }))
}
