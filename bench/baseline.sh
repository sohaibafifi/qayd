...
# Usage: ./baseline.sh [TIMEOUT_S] [YEAR_DIR] [TAG] [JOBS]
...
python3 bench/common/run.py --dir "$csp_dir" --timeout "$T" --jobs "$JOBS" \
    --cmd "$QAYD -t {t} {f}" --out "$OUT/csp-$TAG.csv"
python3 bench/common/run.py --dir "$cop_dir" --timeout "$T" --jobs "$JOBS" \
