...
SCORE_SCHEMA = "qayd.fastcop.score/v1"
...
def score_instance(
    ...
    for solver in solvers:
        ...
        family = record.get(family_field) if record else None
        invalidated_here = (
            record is not None and family in invalidated.get(solver, set())
        )
        if invalidated_here:
            rows[solver] = {
                "score": 0.0,
    ...
    unsat = [
        ...
        if record.get("status") == "UNSAT" and record.get("proof") is True
    ]
    feasible = [record for record in active if objective(record) is not None]
    ...
    optimum_records = [
        ...
        if record.get("status") == "OPTIMUM" and record.get("proof") is True
    ]
    optimum_values = {objective(record) for record in optimum_records}
    ...
    values = [objective(record) for record in feasible]
...
def score_pool(
    ...
    rows = [
        score_instance(instance, by_instance[instance], selected, invalidated, family_field)
...
def score_records(
    ...
    if mode in ("pool", "both"):
        report["pool"] = score_pool(records, selected, invalidation, family_field)
    if mode in ("pairwise", "both"):
        ...
        for left, right in itertools.combinations(selected, 2):
            ...
            report["pairwise"][name] = score_pool(
                records, [left, right], invalidation, family_field
            )
    return report
...
def main() -> int:
    ...
    try:
        records = load_records(path.resolve() for path in args.results)
